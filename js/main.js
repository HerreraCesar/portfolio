...
comenzar.addEventListener("click", function () {
  ...
  setTimeout(() => {
    document.querySelector(".continue").style.animation = "flash 2s infinite";
    ...
    window.addEventListener("wheel", (e) => {
      ...
      if (progress >= 4000) {
        ...
        document.querySelector("#name").addEventListener("click", () => {
          ...
          //document.querySelector("#name").style.animation = "change 0.5s";
          /* setTimeout(() => {
            document.querySelector("#name").style.fontFamily = fonts[random(fonts.length)];
            document.querySelector("#name").style.animation = "none";
          }, 400); */
        });
        document.querySelector("#profile").addEventListener("click", () => {
          document.querySelector("#profile").style.filter =
            pictureFilters[random(pictureFilters.length)];
        });
      }
      if (progress >= 4250) {
        document.querySelector("#profile").style.width = `${
      ...
      if (progress >= 11625) {
        technologies.style.transform = `translateY(${
          600 + (progress - 11625) / -4
        }px)`;
        back.style.transform = `translateY(${
          500 + (progress - 11625) / -5.85
        }px)`;
      }
      if (progress >= 31625) {
        technologies.style.transform = `translateY(-2500px)`;
      }
    });
  }, 3750);
});
